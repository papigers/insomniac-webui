...
export default function ParameterForm() {
  ...
  return (
    <div className="relative">
      <div className="flex border-b border-gray-200 mb-4 bg-white sticky top-40 z-10 -ml-16 pl-16 -mr-20 pr-20">
        {tabs.map((tab) => (
          <button
            key={tab}
      ...
      <div className="divide-y-2 divide-gray-100 -mt-4">
        {parameters
          .filter((param) => param.category === activeTab)
          .map((param) => (
            <Parameter key={param.name} {...param} onChange={onChange} value={params[param.name]} />
          ))}
        <div className=" bg-white sticky bottom-0 py-6 z-10">
